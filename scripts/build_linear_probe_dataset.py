...
def build_linear_probe_dataset_for_split(
    ...
    per_class_counts: Dict[int, Tuple[int, int]] = {}  # class_id -> (num_pos, num_neg)
    ...
    for class_id in range(num_classes):
        ...
        # Prepare "matched" flags per image for greedy assignment
        matched: Dict[str, np.ndarray] = {
            img_id: np.zeros(len(boxes), dtype=bool)
            for img_id, boxes in gt_dict.items()
        }

        # Sort predictions by score, descending (same as eval_yolo)
        ...
        for img_id, box_pred, score in preds_sorted:
            ...
            if gt_boxes is None or gt_boxes.size == 0:
                # No GT for this image and class: this is definitely a FP
                target = 0
            else:
                ...
                if best_iou >= iou_threshold and not matched[img_id][best_idx]:
                    # This prediction is a true positive and "claims" the GT box
                    target = 1
                    matched[img_id][best_idx] = True
                else:
                    # Either IoU too low, or GT already claimed by a higher-score prediction
                    target = 0
